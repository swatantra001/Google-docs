							...
							return (
								<div key={marker} className="absolute bottom-0" style={{ left: `${position}px` }}>
									...
									{marker % 5 === 0 && marker % 10 !== 0 && (
										<div className="absolte bottom-0 w-[1px] h-1.5 bg-neutral-500" />
									)}
									{marker % 5 !== 0 && (
...
const Marker = ({ position, isLeft, isDragging, onMouseDown, onDoubleClick }: MarkerProps) => {
	return (
		...
		>
			...
			<div
				className='absolute left-1/2 top-4 transform -translate-x-1/2 border-l border-dotted  transition-opacity duration-150'
				style={{
					height: "100vh",
					width: "1px", // Keep a 1px width for the border to occupy
					borderLeft: "1.5px dotted #3b72f6", // Correct syntax and slightly thicker for visibility
